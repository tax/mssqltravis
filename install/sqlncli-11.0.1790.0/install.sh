...
bin_dir="";
lib_dir="";
sup_dir="/tmp/microsoft/$driver_short_name/$driver_version";
rll_dir="";
doc_dir="";
inc_dir="";
...
function print_usage()
{
    ...
    echo "      defaults to the /tmp/microsoft/sqlncli/lib directory"
...
function create_symlinks
{
    ...
    if [ $has_aptitude -eq 0 ] && [ "$os_id" == "Ubuntu" ] && [ "$os_release" == "12.04" ]; then
        ...
        log "ldd /tmp/microsoft/sqlncli/lib64/libsqlncli-11.0.so.1790.0"
    fi
...
function install()
{
    ...
    if [ $verified -eq 0 ] || [ $force -eq 1 ]; then
        ...
        if [ "$lib_dir" == "" ]; then
            lib_dir="/tmp/microsoft/$driver_short_name/lib64"
        fi

        bin_dir="/tmp/microsoft/$driver_short_name/bin"
